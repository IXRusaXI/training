...
export function showDevicesBlock(hidden) {
  if (!hidden) {
    // Скрыть (сейчас открыто)
    for (let hiddenLink of collection.deviceHiddenLinksList) {
      hiddenLink.classList.add('hidden');
    }

    collection.deviceShowButton.textContent = 'Показать все';

    collection.deviceDropDown.classList.remove('drop-down-area--open'); // Если класс используется
    collection.deviceHidden = true;
  } else {
    // Показать все (сейчас скрыто)
    for (let hiddenLink of collection.deviceHiddenLinksList) {
      hiddenLink.classList.remove('hidden');
    }

    collection.deviceShowButton.textContent = 'Скрыть';

    collection.deviceDropDown.classList.add('drop-down-area--open'); // Если класс используется
    collection.deviceHidden = false;
  }
}
